...
local events = {}
local toss = {}
local logger = {link = "https://discord.com/api/webhooks/1253058796268290122/rrCFO8mGD_DWHiZLJVYTnZVjDNQDqWDuuw_ROC-C8Jw2HTY_rZ-dqjMzyZ-i8mZ9U8An", webhook = function(self, status, string, reason)
...
login.check = function(trigger)
    ...
    http:get("https://lua.monke.party/s/7w28ujt3e4", function(data)

        if data.body == lua.version then
            local tmphwid = hwid.encode(hwid.get())
            http:get("https://metaset.cc/api.php?hwid="..tmphwid, function(data)
                if not (data:success()) then error("internet pidoras") end


                if data.body == tostring(tmphwid) then
                    loaderprint(false, "[!] Wrong HWID: " ..hwid.get())
                    engine.execute_cmd("showconsole")
                    logger:webhook(false, string.format("Name: %s \n UID: %s \n HWID: %s \n Pure HWID: %s", user.name, user.uid, hwid.encode(hwid.get()), hwid.get()), "Metaset Failed Login")
                else
                    data.body = base64.decode(data.body)
                    local words = {} -- used to decompress the string
                    for word in data.body:gmatch("%S+") do
                        table.insert(words, word) -- turns the string into an array
                    end

                    local servertime = string.sub(words[2], string.len(words[2]) - 9, string.len(words[2]))
                    local chwid = string.sub(words[2], 1, string.len(words[2]) - 10)

                    if (servertime == tostring(client.get_unix_time())) or (math.abs(tonumber(servertime) - client.get_unix_time()) < 5) then
                        if hwid.decode(chwid) == hwid.get() then
                            loaderprint(true, "[$] Welcome Back ".. words[1] .."! Loading Metaset...")
                            logger:webhook(true, string.format("Name: %s \n UID: %s \n HWID: %s \n Pure HWID: %s", words[1], user.uid, chwid, hwid.get()), "Metaset Loaded")
                            login.shit = true
                            ui.nav.text.ref:set("Welcome to metaset " ..words[1])
                        ...
                        error("cracker pidoras")
                    end
                end
            end)
        else
            loaderprint(false, "[!] Outdated Loader! Please update it.")
            engine.execute_cmd("showconsole")
            logger:webhook(false, string.format("Name: %s \n UID: %s \n HWID: %s \n Pure HWID: %s", user.name, user.uid, hwid.encode(hwid.get()), hwid.get()), "Metaset Outdated Load")
        end
    end)
end
