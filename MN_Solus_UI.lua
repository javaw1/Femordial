...
kb.act = {}
kb.list = {
    ["MIN DMG"] = get_binds.min_damage,
    ["Double Tap"] = get_binds.doubletap,
    ["Edge jump"] = get_binds.edge_jump,
    ["BAIM"] = get_binds.baim,
    ["OSAA"] = get_binds.os_aa,
    ["AA Invert"] = get_binds.inverter,
    ["Fake Duck"] = get_binds.fake_duck,
    ["Autopeek"] = get_binds.peek_assist,
    ["Slow Walk"] = get_binds.slow_walk,
    ["Freestand"] = get_binds.freestand,
    ["PING"] = get_binds.ping_spike,
    ["LEAN"] = get_binds.roll,
}
...
call.watermarker = function()
    ...
    local text_changer = (dat:get() and ((el.prefix:get() == "" or #el.prefix:get() > 20) and "M" or el.prefix:get()) or "M")
    ...
    local wide = calc_size(fonts.verdana, tostring((dat:get() and ((el.prefix:get() == "" or #el.prefix:get() > 20) and "M" or el.prefix:get()) or "M") .. (dat:get() and ((el.prefix:get() == "" or #el.prefix:get() > 20) and "N" or el.prefix:get()) or "N")))
    render_text(fonts.verdana, text_changer, vec2_t(var+4, ty), color_t(255,255,255,255))

    wide = calc_size(fonts.verdana, text_changer)
    var = var + wide.x
    text_changer = (dat:get() and ((el.prefix2:get() == "" or #el.prefix2:get() > 20) and "N" or el.prefix2:get()) or "N")
